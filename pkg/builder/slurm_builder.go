...
var (
	...
	nTasksOrNTasksPerNodeMustBeSpecifiedWithNodesErr = errors.New("ntasks or ntasks-per-node must be specified with nodes")
	invalidNodesNTasksOrNTasksPerNodeValueErr        = errors.New("invalid nodes, ntasks or ntasks-per-node value")
	invalidNTasksOrNTasksPerNodeValueErr             = errors.New("invalid ntasks or ntasks-per-node value")
	invalidNodesOrNTasksValueErr                     = errors.New("invalid nodes or ntasks value")
)
...
func (b *slurmBuilder) build(ctx context.Context) (runtime.Object, []runtime.Object, error) {
	if len(b.script) == 0 {
		return nil, nil, noScriptSpecifiedErr
	}
